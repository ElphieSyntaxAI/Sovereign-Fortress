...
export function InfoTip({
  ...
  return (
    <span className="group/info relative inline-flex align-middle">
      <button
        type="button"
        className="ml-1 inline-flex h-5 w-5 items-center justify-center rounded-full border border-slate-600/80 bg-slate-800/80 text-[11px] font-semibold text-cyan-200/90 transition hover:border-cyan-500/40 hover:bg-cyan-500/10 focus:outline-none focus:ring-2 focus:ring-cyan-400/40"
        aria-label={label}
      >
        ⓘ
      </button>
      <span
        role="tooltip"
        className="pointer-events-none absolute left-1/2 top-full z-50 mt-2 w-[min(20rem,calc(100vw-2rem))] -translate-x-1/2 rounded-xl border border-slate-700/80 bg-slate-950/95 p-3 text-left text-xs leading-relaxed text-slate-300 opacity-0 shadow-xl transition duration-200 group-hover/info:pointer-events-auto group-hover/info:opacity-100 group-focus-within/info:pointer-events-auto group-focus-within/info:opacity-100 sm:w-72"
      >
        {children}
      </span>
    </span>
...
export function CopyField({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <pre
        className={`max-h-56 overflow-auto rounded-lg border border-slate-700/60 bg-slate-950/80 p-3 text-xs text-emerald-100/90 ${mono ? "font-mono" : ""}`}
      >
        {value}
      </pre>
    </div>
...
export const PULSE_GUARD_TRACKING = {
  extensions: [
    ".ts",
    ".tsx",
    ".js",
