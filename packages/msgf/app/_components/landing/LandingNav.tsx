...
type Props = {
  /** When set, show the signed-in user bar. */
  userEmail?: string | null;
  showAdminPortalLink?: boolean;
};

export function LandingNav({ userEmail = null, showAdminPortalLink = false }: Props) {
  if (userEmail?.trim()) {
    return (
      <DashboardNav userEmail={userEmail} showAdminPortalLink={showAdminPortalLink} />
    );
  }

  return (
    <header className="sticky top-0 z-50 border-b border-violet-500/10 bg-slate-950/60 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-6xl flex-nowrap items-center justify-between gap-4 px-5">
        ...
        <nav className="flex flex-nowrap items-center justify-end gap-1 sm:gap-2" aria-label="Site">
              <Link
                href="/features"
