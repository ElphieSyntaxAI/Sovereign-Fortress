...
/**
 * MSGF admin SSO handoff — do not read existing browser cookies (large cross-subdomain
 * MSGF session chunks can break setSession or exceed proxy header limits).
 */
export function createBffSupabaseHandoffClient(req: Request, res: Response) {
  loadMonorepoRootEnv();
  const supabaseUrl =
    process.env.NEXT_PUBLIC_SUPABASE_URL?.trim() || process.env.SUPABASE_URL?.trim();
  const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY?.trim();
