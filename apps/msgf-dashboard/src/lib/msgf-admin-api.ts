...
function operatorUserIdHeader(): string | undefined {
  const id = import.meta.env.VITE_MSGF_OPERATOR_USER_ID?.trim();
  return id || undefined;
}
...
export function adminAuthHeaders(extra?: Record<string, string>): HeadersInit {
  ...
  const op = operatorUserIdHeader();
  if (op) {
    headers["x-msgf-operator-user-id"] = op;
  }
  return headers;
}
