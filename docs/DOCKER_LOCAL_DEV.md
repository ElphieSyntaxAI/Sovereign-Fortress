...
REDIS_URL=redis://redis:6379   # optional; compose provides Redis on localhost:6379
```
...
| `npm run docker:dev` | Redis + Author BFF + Author UI + MSGF |
| `npm run docker:dev:author` | Redis + Author only (no MSGF) |
...
| MSGF admin | http://127.0.0.1:3001/admin/sign-in |
