...
- **P7 deferred:** Shadow Eval stores hashed promote/block hits on `msgf_shadow_evaluation_logs` (`p7_deferred`). Reputation is **not** mutated during the 7-day window. **Start 3-day full access** applies those hits once (`p7_applied_at IS NULL` claim) so Active starts warm. Full P7 SSoT: [`MSGF_P7_SOURCE_AUDIT.md`](./MSGF_P7_SOURCE_AUDIT.md).
