...
- [x] `POST /api/v1/chat/completions` shadow mode with valid `x-msgf-key` → projected eval row — 2026-09-22 OpenAI `/v1/chat/completions` 200 (`gpt-4o-mini`) + Anthropic `/v1/messages` 200; dashboard `recent` has projected rows
...
| Date | Note |
| :--- | :--- |
| 2026-09-22 | **Staging AI keys** from `packages/msgf/.env.local` on `msgf-api-staging-00009-bvf` (`ANTHROPIC_API_KEY`, `XAI_API_KEY`, `GCP_API_KEY` / model / location). No `OPENAI_API_KEY` in that file. Shadow/Active `/api/v1/messages` 200 with Haiku 4.5; shadow-eval panel has projected rows. Did not overwrite Secret Manager `MASTER_*` mounts. Soft-RC stays **~90%**. |
| 2026-09-22 | **Staging smoke partial** on `https://staging.elphiesgatedai.elphiesyntax.com` (`msgf-api-staging-00008-np6`, tenant `staging_readiness`). Verified: `probe:solo` (Pulse/heal-queue), ingest + `lineage_map`, user heal-queue, admin queue load, dashboard live, period + PDF, buyer foreign 403, Active `x-msgf-routing` + spoof ignored, swarm 409 / no key lists, heartbeat dry_run 200, audit hub `p7=` chips. Still open: Shadow 3-day CTA / `p7_applied_at`, Sentry 503, `/setup/projects`, HITL POST 500 `arbitrate_audit_id`. Soft-RC stays **~90%**; paid stays **~82%**. |
